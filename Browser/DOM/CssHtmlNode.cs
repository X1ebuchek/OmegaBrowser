...
public class CssHtmlNode : HtmlNode
{
    public CSSAttrMap cssAttrMap { get; }
    internal CssHtmlNode _parentnode;
    private bool _changed;

    public CssHtmlNode(HtmlNodeType type, HtmlDocument ownerdocument, int index) : base(type, ownerdocument, index)
    {
        cssAttrMap = FillDefaultCssAttributes(base.Name);
    }

    public CssHtmlNode(HtmlNode node) : base(node.NodeType, node.OwnerDocument, node.OuterStartIndex)
    {
        cssAttrMap = FillDefaultCssAttributes(node.Name);
    }

    public new CssHtmlNode ParentNode
    {
